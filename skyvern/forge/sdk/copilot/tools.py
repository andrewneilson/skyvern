...
from skyvern.forge.sdk.copilot.dispatcher import NativeTool, SchemaOverlay, ToolDispatcher
...
async def _run_blocks_and_collect_debug(params: dict[str, Any], ctx: AgentContext) -> dict[str, Any]:
    ...
    if final_status is None:
        ...
        return {
            "ok": False,
            "error": f"Block execution timed out after {max_poll}s. Run ID: {workflow_run.workflow_run_id}",
        }
    ...
    return {
        ...
        "data": {
            ...
            "blocks": results,
            "visible_elements_html": html[:5000] if html else None,
...
async def _fallback_page_info(ctx: AgentContext) -> tuple[str, str]:
    """Return (url, title) from the browser session, or ("", "") on failure."""
    ...
    return {
        ...
        "click": SchemaOverlay(
            description=(
                ...
                "Use this for precise interactions when you know the exact element to target."
            ),
...
def register_all_tools(dispatcher: ToolDispatcher) -> None:
        ...
        NativeTool(
            ...
            schema={
                ...
                "properties": {
                    "workflow_yaml": {
                        "type": "string",
                        "description": "Complete workflow YAML definition string.",
                    },
                },
    ...
    dispatcher.register_native(
        ...
        NativeTool(
            handler=_run_blocks_and_collect_debug,
            schema={
                "description": (
                ...
                "properties": {
                    "block_labels": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "List of block labels to run (in order).",
                    },
                    "parameters": {
                        "type": "object",
                        "description": (
                            "Values for workflow parameters referenced by the blocks "
                            '(e.g. {"search_query": "test value"}). '
                            "Any workflow parameters not supplied here will be auto-filled "
                            "with type-appropriate defaults so testing is not blocked."
                        ),
                    },
                },
                "required": ["block_labels"],
            },
            timeout=180,
        ),
    )
    ...
    dispatcher.register_native(
        "get_run_results",
        NativeTool(
            handler=_get_run_results,
            schema={
                "description": (
                    "Fetch results from a previous workflow run. "
                    "Returns block statuses, failure reasons, and output data (e.g. extraction results). "
                    "If workflow_run_id is omitted, fetches the most recent completed run for this workflow. "
                    "Use this when the user reports problems with output (duplicates, missing data, errors) "
                    "to inspect what the workflow actually produced."
                ),
                "properties": {
                    "workflow_run_id": {
                        "type": "string",
                        "description": "Specific workflow run ID to inspect. If omitted, uses the most recent run.",
                    },
                },
                "required": [],
            },
    ...
    if set(alias_map.keys()) != set(overlays.keys()):
        missing_aliases = sorted(set(overlays.keys()) - set(alias_map.keys()))
        missing_overlays = sorted(set(alias_map.keys()) - set(overlays.keys()))
        raise RuntimeError(
            f"MCP overlay/alias mismatch. missing_aliases={missing_aliases}, missing_overlays={missing_overlays}"
        )

    for copilot_name, overlay in overlays.items():
        dispatcher.register_overlay(copilot_name, overlay)
