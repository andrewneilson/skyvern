...
async def _restore_workflow_on_error(original_workflow: Workflow | None, organization_id: str) -> None:
    """Restore the original workflow definition if the agent persisted intermediate versions.

    Called on disconnect, when auto-accept is off, and on error paths.
    """
...
async def workflow_copilot_chat_post(
    ...
    async def stream_handler(stream: EventSourceStream) -> None:
        ...
        try:
            ...
            # Restore raw YAML from proposed_workflow if available, so the
            # agent sees the last-proposed definition instead of empty blocks.
            ...
            # Snapshot current workflow definition before agent loop so we can
            # restore it if the user hasn't opted into auto-accept.
            ...
            # Override request's workflow_id with the canonical one from DB
            # so AgentContext.workflow_id always matches the permanent_id.
            ...
            # Extract API key from request for MCP delegation
            ...
            if chat.auto_accept is not True:
                if agent_result.workflow_was_persisted:
                    await _restore_workflow_on_error(original_workflow, organization.organization_id)
                if updated_workflow:
