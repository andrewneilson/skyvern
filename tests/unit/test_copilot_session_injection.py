...
class TestSanitization:
    def test_screenshot_sanitization(self) -> None:
        from skyvern.forge.sdk.copilot.agent import _sanitize_tool_result_for_llm
        ...
        sanitized = _sanitize_tool_result_for_llm("get_browser_screenshot", result)
    ...
    def test_mcp_fields_stripped(self) -> None:
        ...
        sanitized = _sanitize_tool_result_for_llm("navigate_browser", result)
    ...
    def test_summarize_type_text_accepts_both_field_names(self) -> None:
        from skyvern.forge.sdk.copilot.agent import _summarize_tool_result

        # Copilot field name
        result1 = {"ok": True, "data": {"selector": "#email", "typed_length": 10}}
        assert "10" in _summarize_tool_result("type_text", result1)

        # MCP field name
        result2 = {"ok": True, "data": {"selector": "#email", "text_length": 20}}
        assert "20" in _summarize_tool_result("type_text", result2)
    ...
    def test_summarize_navigate_reads_top_level_url(self) -> None:
        ...
        summary = _summarize_tool_result("navigate_browser", result)
...
class TestLastWorkflowClearedOnRunFailure:
    @pytest.mark.asyncio
    async def test_run_failure_clears_last_workflow(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from dataclasses import dataclass

        from skyvern.forge.sdk.copilot.agent import _run_agent_loop
        from skyvern.forge.sdk.copilot.dispatcher import NativeTool, ToolDispatcher
        from skyvern.forge.sdk.copilot.mcp_host import MCPHost

        @dataclass
        class _FakeChatRequest:
            workflow_id: str = "wf-1"
            workflow_permanent_id: str = "wfp-1"
            workflow_yaml: str = "title: Test"
            message: str = "build me a workflow"

        chat_request = _FakeChatRequest()

        stream = _make_stream()
        stream.send = AsyncMock()

        fake_workflow = MagicMock()
        fake_workflow.title = "Test"

        host = MagicMock(spec=MCPHost)
        host.get_discovered_tools.return_value = {}

        async def _handle_update_workflow(inp: dict[str, Any], ctx: Any) -> dict[str, Any]:
            return {
                "ok": True,
                "data": {"block_count": 1},
                "_workflow": fake_workflow,
            }

        async def _handle_run_blocks(inp: dict[str, Any], ctx: Any) -> dict[str, Any]:
            return {"ok": False, "error": "Block execution failed"}
        ...
        dispatcher.register_native(
            ...
            NativeTool(
                ...
                schema={"description": "update", "properties": {}, "required": []},
            ),
        )
        dispatcher.register_native(
            "run_blocks_and_collect_debug",
            NativeTool(
                handler=_handle_run_blocks,
                schema={"description": "run", "properties": {}, "required": []},
            ),
        )
        ...
        async def fake_llm_call(**kwargs: Any) -> dict[str, Any]:
                    ...
                    "content": [
                        {
                            ...
                            "input": {"workflow_yaml": "title: Test"},
                        }
                    ],
        ...
        fake_caller = MagicMock(
            call=AsyncMock(side_effect=fake_llm_call),
            message_history=[],
        )

        monkeypatch.setattr(
            "skyvern.forge.sdk.copilot.agent._estimate_tokens",
            lambda msgs, model: 1000,
        )

        ctx = _make_ctx(browser_session_id=None)

        result = await _run_agent_loop(
            caller=fake_caller,
            ctx=ctx,
            dispatcher=dispatcher,
            tool_defs=dispatcher.get_tool_definitions(tool_format="anthropic"),
            stream=stream,
            organization_id="org-1",
        ...
        assert call_counter == 3, f"Expected 3 LLM calls, got {call_counter}"
        assert result.updated_workflow is None, (
            "updated_workflow should be None after run_blocks_and_collect_debug fails"
        )
